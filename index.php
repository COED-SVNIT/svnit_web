<!DOCTYPE html>
<html lang="en">
<head>
  <link rel="icon" type="image/png" href="svnit.png">
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8"/>
  <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=no"/>
  <title>SVNIT</title>

  <!-- CSS  -->
  <link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet">
  <link href="css/materialize.css" type="text/css" rel="stylesheet" media="screen,projection"/>
  <link href="css/style.css" type="text/css" rel="stylesheet" media="screen,projection"/>
  <link rel="stylesheet" type="text/css" href="css/newsTick.css">
<style type="text/css">
  
  #crdtop{
    height: 140px;
    cursor: pointer;
  }
  #crdbottom{height: 60px;}

  .boards{
    padding-top: 2%; 
  }
</style>

</head>
<body>
<ul id="student" class="dropdown-content black-text">
<li><a href="#!">Admission</a></li>
<li><a href="#!">Current Student</a></li>
<li class="divider"></li>
<li><a href="#!">Anti-Ragging</a></li>
<li><a href="#!">Online Fees</a></li>
<li><a href="#!">Physical Education</a></li>
</ul>
<ul id="department" class="dropdown-content">
<li><a href="#!">Applied Mechanics</a></li>
<li><a href="#!">Applied Chemistry</a></li>
<li><a href="#!">Applied Math. & Human.</a></li>
<li><a href="#!">Applied Physics</a></li>
<li><a href="#!">Civil Engineering</a></li>
<li><a href="#!">Chemical Engineering</a></li>
<li><a href="#!">Computer Engineering</a></li>
<li><a href="#!">Electrical Engineering</a></li>
<li><a href="#!">Electronics Engineering</a></li>
<li><a href="#!">Mechanical Engineering</a></li>
</ul>
<ul id="campus" class="dropdown-content">
<li><a href="#!">About Campus</a></li>
<li><a href="#!">Campus Map</a></li>
<li><a href="#!">Canteen</a></li>
<li><a href="#!">Day Care Center</a></li>
<li><a href="#!">Guest House</a></li>
<li><a href="#!">Health Center</a></li>
<li><a href="#!">Banks</a></li>
<li><a href="#!">Post Office</a></li>
<li><a href="#!">Security</a></li>
<li><a href="#!">CCC</a></li>
</ul>
<ul id="centralfacility" class="dropdown-content">
<li><a href="#!">Web Mail</a></li>
<li><a href="#!">MIS</a></li>
<li><a href="#!">Moodle</a></li>
<li><a href="#!">E-Library</a></li>
<li><a href="#!">Phonebook</a></li>
</ul>
<ul id="media" class="dropdown-content">
<li><a href="#!">News Hub</a></li>
<li><a href="#!">Photo Gallery</a></li>
<li><a href="#!">Magazines</a></li>
</ul>
<ul id="slide-out" class="side-nav">
  <li><a href="#!">Home</a></li>
  <li class="no-padding">
    <ul class="collapsible collapsible-accordion">
      <li>
        <a class="collapsible-header">Student<i class="material-icons">arrow_drop_down</i></a>
        <div class="collapsible-body">
          <ul>
            <li><a href="#!">Admission</a></li>
            <li><a href="#!">Current Student</a></li>
            <li class="divider"></li>
            <li><a href="#!">Anti-Ragging</a></li>
            <li><a href="#!">Online Fees</a></li>
            <li><a href="#!">Physical Education</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </li>
  <li class="no-padding">
    <ul class="collapsible collapsible-accordion">
      <li>
        <a class="collapsible-header">Campus<i class="material-icons">arrow_drop_down</i></a>
        <div class="collapsible-body">
          <ul>
            <li><a href="#!">About Campus</a></li>
            <li><a href="#!">Campus Map</a></li>
            <li><a href="#!">Canteen</a></li>
            <li><a href="#!">Day Care Center</a></li>
            <li><a href="#!">Guest House</a></li>
            <li><a href="#!">Health Center</a></li>
            <li><a href="#!">Banks</a></li>
            <li><a href="#!">Post Office</a></li>
            <li><a href="#!">Security</a></li>
            <li><a href="#!">CCC</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </li>
  <li class="no-padding">
    <ul class="collapsible collapsible-accordion">
      <li>
        <a class="collapsible-header">Department<i class="material-icons">arrow_drop_down</i></a>
        <div class="collapsible-body">
          <ul>
            <li><a href="#!">Applied Mechanics</a></li>
            <li><a href="#!">Applied Chemistry</a></li>
            <li><a href="#!">Applied Math. & Human.</a></li>
            <li><a href="#!">Applied Physics</a></li>
            <li><a href="#!">Civil Engineering</a></li>
            <li><a href="#!">Chemical Engineering</a></li>
            <li><a href="#!">Computer Engineering</a></li>
            <li><a href="#!">Electrical Engineering</a></li>
            <li><a href="#!">Electronics Engineering</a></li>
            <li><a href="#!">Mechanical Engineering</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </li>
  <li class="no-padding">
    <ul class="collapsible collapsible-accordion">
      <li>
        <a class="collapsible-header">Central Facility<i class="material-icons">arrow_drop_down</i></a>
        <div class="collapsible-body">
          <ul>
            <li><a href="#!">Web Mail</a></li>
            <li><a href="#!">MIS</a></li>
            <li><a href="#!">Moodle</a></li>
            <li><a href="#!">E-Library</a></li>
            <li><a href="#!">Phonebook</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </li>
  <li class="no-padding">
    <ul class="collapsible collapsible-accordion">
      <li>
        <a class="collapsible-header">Media<i class="material-icons">arrow_drop_down</i></a>
        <div class="collapsible-body">
          <ul>
            <li><a href="#!">News Hub</a></li>
            <li><a href="#!">Photo Gallery</a></li>
            <li><a href="#!">Magazines</a></li>
          </ul>
        </div>
      </li>
    </ul>
  </li>
  <li><a href="#!">Careers</a></li>
</ul>
<div class="navbar-fixed">
  <nav class="white" role="navigation">
    <div class="nav-wrapper ">

      <a href="#!" class="brand-logo" style="padding-top:5px"><img class="responsive-img" src="svnit1.png"></a>
      <ul class="right hide-on-med-and-down">
        <li><a href="#">Home</a></li>
        <li><a class="dropdown-button" href="#!" data-activates="student">Student<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a class="dropdown-button" href="#!" data-activates="campus">Campus<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a class="dropdown-button" href="#!" data-activates="department">Department<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a class="dropdown-button" href="#!" data-activates="centralfacility">Central Facility<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a class="dropdown-button" href="#!" data-activates="media">Media<i class="material-icons right">arrow_drop_down</i></a></li>
        <li><a href="#">Careers</a></li>
      </ul>
      <a href="#" data-activates="slide-out" class="button-collapse black-text"><i class="material-icons">menu</i></a>

    </div>
  </nav>
</div>

  <div id="index-banner" class="parallax-container">
    <div class="section no-pad-bot">
      <div class="container">
        <br><br>
        <h1 class="header center white-text text-lighten-2 flow-text">Sardar Vallabhbhai National Institute Of Technology</h1>
        <div class="row center">
          <h5 class="header col s12 light flow-text">An Institute of National Importance since 1961.</h5>
        </div>
        <br><br>

      </div>
    </div>
    <div class="parallax"><img src="background1.jpg" alt="Unsplashed background img 1"></div>
  </div>





<div >
  <div class="row boards ">
    
    <div class="col l4 m4 s12 center-align">
    <div class="card-panel black">
      <span class=" white-text "><B> Student Notice</B></span>
    </div>
    <div id="nt-1-container" class="nt-container">
      <ul class="newsticker" id="nt-1">

          <li>Binary letters, 1110101011<a href="#">110101111101011</a> 00010101100</li>
          <li>Piyush  is my name <a href="#">ut condimentum lectus</a> piyush piyush.</li>
          <li>arshit himanshu , <a href="#">ut condimentum lectus</a> himanshu arshit.</li>
          <?php
                  require 'database.php';

                  $query = 'SELECT * FROM notice_events order by notice_event_id DESC';
                  $result = mysqli_query($conn,$query);


                    while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                      date_default_timezone_set('Asia/Kolkata');
                        $date = date("Y-m-d");
                                
                        $notice_event = $row['notice_event'];
                        $end_date = $row['end_date'];
                        if(($notice_event == "1") && ($end_date >= $date)) {
                        $title = $row['title'];
                        $desc = $row['description'];
                        $link = $row['link'];
                        // echo '<script>alert("'.$link.'"); </script>';
                        $imp = $row['important'];
                        echo '<li><table cellpadding="4">';
                        echo '<tr><td>'.$title.' | '.'<a href="'.$link.'">'.$desc.'</a></td></tr></table></li>';
                      }
                        }


              ?>
      </ul>
      <i class="small material-icons" id="nt-1-prev">arrow_downward</i>
      <i class="small material-icons" id="nt-1-next" >arrow_upward</i>
      </div>
    </div>

   
     <div class="col l4 m4 s12  center-align">
    <div class="card-panel black">
      <span class="white-text"><b>Events & Seminars</b></span>
    </div>
    <div id="nt-2-container" class="nt-container">
      <ul class="newsticker" id="nt-2">
          <li>Binary letters, 1110101011<a href="#">110101111101011</a> 00010101100</li>
          <li>Piyush  is my name <a href="#">ut condimentum lectus</a> piyush piyush.</li>
          <li>arshit himanshu , <a href="#">ut condimentum lectus</a> himanshu arshit.</li>
          <?php
                    $query = 'SELECT * FROM notice_events order by notice_event_id DESC';
                    $result = mysqli_query($conn,$query);

                    while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                      date_default_timezone_set('Asia/Kolkata');
                        $date = date("Y-m-d");
                                
                        $notice_event = $row['notice_event'];
                        $end_date = $row['end_date'];
                        if(($notice_event == "0") && ($end_date >= $date)) {
                        $title = $row['title'];
                        $desc = $row['description'];
                        $link = $row['link'];
                        // echo '<script>alert("'.$link.'"); </script>';
                        $imp = $row['important'];
                        echo '<li class="news-item"><table cellpadding="4">';
                        echo '<tr><td>'.$title.' | '.'<a href="'.$link.'">'.$desc.'</a></td></tr></table></li>';
                      }
                        }


              ?>
      </ul>
      <i class="small material-icons" id="nt-2-prev">arrow_downward</i>
      <i class="small material-icons" id="nt-2-next" >arrow_upward</i>
      </div>
    </div>




    <div class="col l4 m4 s12  center-align">
    <div class="card-panel black">
      <span class="white-text "><b>Tenders & Enquiries</b></span>
    </div>
    <div id="nt-3-container" class="nt-container">
      <ul class="newsticker" id="nt-3">
          <li>Binary letters, 1110101011<a href="#">110101111101011</a> 00010101100</li>
          <li>Piyush  is my name <a href="#">ut condimentum lectus</a> piyush piyush.</li>
          <li>arshit himanshu , <a href="#">ut condimentum lectus</a> himanshu arshit.</li>
        <?php
                    $query = 'SELECT * FROM notice_events order by notice_event_id DESC';
                    $result = mysqli_query($conn,$query);

                    while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
                      date_default_timezone_set('Asia/Kolkata');
                        $date = date("Y-m-d");
                                
                        $notice_event = $row['notice_event'];
                        $end_date = $row['end_date'];
                        if(($notice_event == "2") && ($end_date >= $date)) {
                        $title = $row['title'];
                        $desc = $row['description'];
                        $link = $row['link'];
                        // echo '<script>alert("'.$link.'"); </script>';
                        $imp = $row['important'];
                        echo '<li class="news-item"><table cellpadding="4">';
                        echo '<tr><td>'.$title.' | '.'<a href="'.$link.'">'.$desc.'</a></td></tr></table></li>';
                      }
                        }


              ?>
      </ul>
      <i class="small material-icons" id="nt-3-prev">arrow_downward</i>
      <i class="small material-icons" id="nt-3-next" >arrow_upward</i>
      </div>
    </div>
  

  </div>
</div>














  <div class="parallax-container valign-wrapper">
    <div class="section no-pad-bot">
      <div class="container">
        <div class="row center">
          <h5 class="header col s12 light">Infrastructure at it's best</h5>
        </div>
      </div>
    </div>
    <div class="parallax"><img src="background2.jpg" alt="Unsplashed background img 2"></div>
  </div>





  <!-- card -->
<div  style="margin:0px;" class="section card-panel white">

      <div class="row center-align">
      
        <div class="col s12 m4 l2">
          <div class="card  hoverable" >
            <div id="crdtop" class="card-content black white-text">
              <span class="card-title">Events</span>
              <p>hey Events here.</p>
            </div>
            <div id="crdbottom" class="card-action grey lighten-2" >
              <a  style="color:black;"  href="#modal1" >Explore</a>
          <div id="modal1" class="modal bottom-sheet">
        <div class="modal-content">
          <ul class="collection with-header">
                  <li class="collection-header"><center><h4 class=" light-green-text text-darken-3">Svnit's Events</h4></center></li>
                 <li class="collection-item avatar">
                <i class="material-icons circle red">play_arrow</i>
                <span class="title"><h5 class="blue-text text-darken-2">Sparsh</h5></span>
                <p>Annual Techanical<br>
                 And Cultural Fest
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
               <li class="collection-item avatar">
                <i class="material-icons circle red">play_arrow</i>
                <span class="title" ><h5 class="blue-text text-darken-2">MMNCT</h5></span>
                <p>Annual<br>
                 Night Cricket Turnament
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
          </ul>
        </div>
        <div class="modal-footer">
          <a href="#!" class=" modal-action modal-close waves-effect waves-green btn-flat">Cancel</a>
        </div>
      </div>
            </div>
    
          </div>
        </div>
      <div class="col s12 m4 l2">
        <div class="card hoverable" >
          <div  id="crdtop" class="card-content black white-text ">
            <span class="card-title">Student Chapters</span>
            <p></p>
          </div>
          <div id="crdbottom" class="card-action grey lighten-2">
            <a  href="#modal2" style="color:black;">chapters</a>
      <div id="modal2" class="modal bottom-sheet">
        <div class="modal-content">
          <ul class="collection with-header">
                  <li class="collection-header"><center><h4 class=" light-green-text text-darken-3">Student Chapters</h4></center></li>
                 <li class="collection-item avatar">
                <i class="material-icons circle green">android</i>
                <span class="title"><h5 class="blue-text text-darken-2">ACM</h5></span>
                <p>Computer department
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
               <li class="collection-item avatar">
                <i class="material-icons circle red">group_work</i>
                <span class="title" ><h5 class="blue-text text-darken-2">Drishti</h5></span>
                <p>
                Electronics And Communication Department
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
               <li class="collection-item avatar">
                <i class="material-icons circle yellow">settings_input_composite</i>
                <span class="title" ><h5 class="blue-text text-darken-2"></h5></span>
                <p>
                Electrical Department
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
              <li class="collection-item avatar">
                <i class="material-icons circle  brown">invert_colors</i>
                <span class="title" ><h5 class="blue-text text-darken-2"></h5></span>
                <p>
                Chemical Department
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
              <li class="collection-item avatar">
                <i class="material-icons circle blue">business</i>
                <span class="title" ><h5 class="blue-text text-darken-2">CES</h5></span>
                <p>
                Civil Department
                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
                <li class="collection-item avatar">
                <i class="material-icons circle orange">track_changes</i>
                <span class="title" ><h5 class="blue-text text-darken-2">SCOSH</h5></span>
                <p>
                Applied Mathematics,Physics,Chemistry & Humanities Department

                </p>
                <a href="#!" class="secondary-content"><i class="material-icons">send</i></a>
              </li>
          </ul>
        </div>
        <div class="modal-footer">
          <a href="#!" class=" modal-action modal-close waves-effect waves-green btn-flat">Cancel</a>
        </div>
      </div>
          </div>
        </div>
      </div>
    <div class="col s12 m4 l2" >
      <div class="card hoverable">
        <div  id="crdtop" class="card-content black white-text ">
          <span class="card-title">E-Library</span>
          <p>hey E-library here.</p>
        </div>
        <div id="crdbottom"  class="card-action grey lighten-2">
          <a href="#" style="color:black;">link</a>
        </div>
      </div>
    </div>
  <div class="col s12 m4 l2">
    <div class="card hoverable" >
      <div id="crdtop" class="card-content black white-text">
        <span class="card-title">TnP</span>
        <p>Companies here.</p>
      </div>
      <div id="crdbottom"  class="card-action grey lighten-2">
        <a href="#" style="color:black;">Schedule</a>
      </div>
    </div>
  </div>
<div class="col s12 m4 l2">
  <div class="card hoverable" >
    <div id="crdtop"  class="card-content black white-text">
      <span class="card-title">TEQIP</span>
      <p>hey TEQIP here.</p>
    </div>
    <div id="crdbottom" class="card-action  grey lighten-2 ">
      <a href="#modal3" style="color:black;">Events Schedule</a>
    </div>
  <div id="modal3" class="modal bottom-sheet">
        <div class="modal-content">
          <ul class="collection with-header">
                  <li class="collection-header"><center><h4 class=" light-green-text text-darken-3">TEQIP 1</h4></center></li>
                <li class="collection-header"><center><h4 class=" light-green-text text-darken-3">TEQIP 2</h4></center></li>
                 
          </ul>
        </div>
        <div class="modal-footer">
          <a href="#!" class=" modal-action modal-close waves-effect waves-green btn-flat">Cancel</a>
        </div>
  </div>
</div>
          </div>
<div class="col s12 m4 l2">
      <div class="card hoverable">
        <div  id="crdtop" class="card-content black white-text ">
          <span class="card-title">Alumni</span>
          <p>hey here.</p>
        </div>
        <div id="crdbottom"  class="card-action grey lighten-2">
          <a href="#" style="color:black;">link</a>
        </div>
      </div>
    </div>
      </div>

    </div>



  <div class="parallax-container valign-wrapper">
    <div class="section no-pad-bot">
      <div class="container">
        <div class="row center">
          <h5 class="header col s12 light">SVNIT's New library</h5>
        </div>
      </div>
    </div>
    <div class="parallax"><img src="background3.jpg" alt="Unsplashed background img 3"></div>
  </div>

  <footer class="page-footer black">
    <div class="container">
      <div class="row">
        <div class="col l6 s12">
          <h5 class="white-text">Sardar Vallabhbhai National Institute of Technology, Surat</h5>
          <p class="grey-text text-lighten-4">The Sardar Vallabhbhai National Institute of Technology, Surat popularly known as National Institute of Technology, Surat, NIT Surat or SVNIT, is an engineering institute of higher education established by the Parliament of India in 1961.</p>


        </div>
        <div class="col l3 s12">
          <h5 class="white-text">Information</h5>
          <ul>
            <li><a class="white-text" href="#!">Right to Information</a></li>
            <li><a class="white-text" href="#!">About</a></li>
            <li><a class="white-text" href="#!">Contact Us</a></li>
            <li><a class="white-text" href="#!">Helpline</a></li>
            <li><a class="white-text" href="#!">IT policy</a></li>
          </ul>
        </div>
        <div class="col l3 s12">
          <h5 class="white-text">Connect</h5>
          <ul>
            <li><a class="white-text" href="#!">Minutes</a></li>
            <li><a class="white-text" href="#!">Tenders</a></li>
            <li><a class="white-text" href="#!">Donation</a></li>
            <li><a class="white-text" href="#!">Other Info</a></li>
          </ul>
        </div>
      </div>
    </div>
    <div class="footer-copyright">
      <div class="container">
      Made by <a class="brown-text text-lighten-3" href="#">Web team</a>
      </div>
    </div>
  </footer>


  <!--  Scripts-->
  <script src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
  <script src="js/materialize.js"></script>
  <script src="js/init.js"></script>
  <script src="js/jquery.newsTicker.js"></script>
<script type="text/javascript">
  $(document).ready(function(){
     // the "href" attribute of .modal-trigger must specify the modal ID that wants to be triggered
     $('.modal').modal();
  
   });

  $('#nt-1').newsTicker({
    row_height: 48,
    max_rows: 4,
    speed: 600,
    direction: 'up',
    duration: 2000,
    autostart: 1,
    pauseOnHover: 1,
    prevButton: $('#nt-1-prev'),
                nextButton: $('#nt-1-next')
});

$('#nt-2').newsTicker({
    row_height: 48,
    max_rows: 4,
    speed: 600,
    direction: 'up',
    duration: 2300,
    autostart: 1,
    pauseOnHover: 1,
    prevButton: $('#nt-2-prev'),
                nextButton: $('#nt-2-next')
});


$('#nt-3').newsTicker({
    row_height: 48,
    max_rows: 4,
    speed: 600,
    direction: 'up',
    duration: 2500,
    autostart: 1,
    pauseOnHover: 1,
    prevButton: $('#nt-3-prev'),
                nextButton: $('#nt-3-next')
});


</script>






  </body>
</html>
